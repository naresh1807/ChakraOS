...
|---|---|---|
| **USB Guard** | The real `usbguard` daemon, bootstrapped to allow what's connected at boot and block new insertions by default — the first thing to actually enforce a `/etc/chakra/policy.d/` policy (Phase 5's `usb-storage.policy.json`). `usbguard-notifier` runs in the session so a blocked insertion is *notified*, not silent. | `core/security/usbguard/` |
...
`usb-storage.policy.json`'s "ask" behavior in Daily mode is now *partly* real: with `usbguard-notifier` a blocked insertion raises a "USB device blocked: `<name>`" desktop notification instead of failing silently. It is still not a one-click prompt — allowing a device is a manual `sudo usbguard allow-device <id>`. A notification carrying Allow / Block / Allow-permanently actions wired to the usbguard IPC is Phase 9 work (see `core/security/usbguard/README.md`), and deliberately deferred: it means granting a user-session component the ability to authorise hardware, which is a security decision, not just UI.
