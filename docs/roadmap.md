...
- **Phase 5 — Security substrate.** nftables firewall (default-deny inbound, permissive outbound — this is a pentesting OS), kernel/network hardening (deliberately excludes `ptrace_scope`, since gdb/strace/ltrace need it), `auditd` for OS-level audit logging, and a Policy Engine *schema* (JSON, risk-tiered, one worked example) — explicitly not a running engine yet, since nothing (Sentinel, AppGuard) exists to consume its decisions. Secure Boot/TPM/disk encryption deferred to when there's an installer and an actual persistent disk to protect. → `core/security/`, `core/policies/`
